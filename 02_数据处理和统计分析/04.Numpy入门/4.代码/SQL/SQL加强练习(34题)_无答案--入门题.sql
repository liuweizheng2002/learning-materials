...
select
    product_name, p.unit_price, quantity, company_name as supplier_name
from
    order_items oi
join
    products p on p.product_id = oi.product_id
...
select
    s.supplier_id, s.company_name,      # 这里可以是分组字段, 也可以是和 分组字段唯一对应的 其他列.
...
select
    c.customer_id, c.company_name, sum(unit_price * quantity * (1 - discount)) total_paid
from
    orders o
join
    order_items oi on o.order_id = oi.order_id
...
from orders where order_id between 10720 and 10730;
...
-- 促销策略, 参见需求21的代码.   北美 -> USA, Canada,   Other -> 其它
# 思路1: 普通版.
select
...
# 思路2: cte写法.
with t1 as (select order_id, case when ship_country in ('USA', 'Canada') then '北美' else 'other' end as new_addr from orders)
...
# 验真: 4,Tokyo Traders,64,29 ->  供应商id, 供应商公司名, 总库存, 高价值库存.
select product_id, supplier_id, unit_price, units_in_stock from products where supplier_id = 4;

explain select * from day02.student where age=38;
explain select * from day02.student where sid=1;
